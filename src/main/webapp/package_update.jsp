...
<h1>Package control</h1>
	...
	<section class="packages">
		...
		<h1 class="heading-title">Edit Packages</h1>


		<div class="box-container">
			<c:forEach var="pck" items="${pack}">
			<c:set var="name" value ="${pck.name}"/>
			<c:set var="details" value ="${pck.details}"/>
			<c:set var="duration" value ="${pck.duration}"/>
			<c:set var="price" value ="${pck.price}"/>
			<c:set var="idpackage" value ="${pck.idpackage}"/>
			<c:set var="image" value ="${pck.image}"/>
				...
				<div class="box">
					...
					<div class="content">
						...
						<c:url value="pack_updateform.jsp" var="packupdate">
						<c:param name="name" value="${pck.name}"/>
						<c:param name="details" value="${pck.details}"/>
						<c:param name="duration" value="${pck.duration}"/>
						<c:param name="price" value="${pck.price}"/>
						<c:param name="idpackage" value="${pck.idpackage}"/>
						<c:param name="image" value="${pck.image}"/>
						</c:url>
						<a href="${packupdate}">
